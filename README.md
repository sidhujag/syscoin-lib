# Simple Scala Syscoin Library

Simple syscoin library written in Scala.

[![Build Status](https://travis-ci.org/ACINQ/syscoin-lib.png)](https://travis-ci.org/ACINQ/syscoin-lib)

## Overview

This is a simple scala library which implements most of the syscoin protocol:
...
Our goal is not to re-implement a full Syscoin node but to build a library that can be used to build applications that rely on syscoind to interface with the Syscoin network (to retrieve and index transactions and blocks, for example...). We use it very often to build quick prototypes and test new ideas. Besides, some parts of the protocole are fairly simple and "safe" to re-implement (BIP32/BIP39 for example), especially for indexing/analysis purposes. And, of course, we use it for our own work on Lightning.
...
<dependencies>
  <dependency>
    ...
    <artifactId>syscoin-lib_2.11</artifactId>
...
Syscoin-lib, starting with version 0.9.7, fully supports segwit (see below for more information) and is on par with the segwit code in Syscoin Core 0.13.1.
...
mvn scala:console
scala> import fr.acinq.syscoin._
import fr.acinq.syscoin._

scala> import fr.acinq.syscoin.Crypto._
import fr.acinq.syscoin.Crypto._

scala> val priv = PrivateKey(BinaryData("1e99423a4ed27608a15a2616a2b0e9e52ced330ac530edcc32c8ffc6a526aedd"), compressed = true)
priv: fr.acinq.syscoin.Crypto.PrivateKey = 1e99423a4ed27608a15a2616a2b0e9e52ced330ac530edcc32c8ffc6a526aedd01

scala> val priv = PrivateKey(BinaryData("1e99423a4ed27608a15a2616a2b0e9e52ced330ac530edcc32c8ffc6a526aedd"), compressed = false)
priv: fr.acinq.syscoin.Crypto.PrivateKey = 1e99423a4ed27608a15a2616a2b0e9e52ced330ac530edcc32c8ffc6a526aedd

scala> val pubUncompressed = priv.publicKey
pubUncompressed: fr.acinq.syscoin.Crypto.PublicKey = 04f028892bad7ed57d2fb57bf33081d5cfcf6f9ed3d3d7f159c2e2fff579dc341a07cf33da18bd734c600b96a72bbc4749d5141c90ec8ac328ae52ddfe2e505bdb
...
pubCompressed: fr.acinq.syscoin.Crypto.PublicKey = 03f028892bad7ed57d2fb57bf33081d5cfcf6f9ed3d3d7f159c2e2fff579dc341a
...
The Transaction class can be used to create, serialize, deserialize, sign and validate syscoin transactions.

#### P2PKH transactions

A P2PKH transactions sends syscoins to a public key hash, using a standard P2PKH script:
``` scala
...
A P2SH transactions sends syscoins to a script hash:
```scala
...
```scala
  ...
  // this is a standard tx that sends 0.5 SYS to D6YX7dpieYu8j1bV8B4RgksNmDk3sNJ4Ap
  ...
  // now let's create a simple tx that spends tx1 and send 0.5 SYS to a P2WSH output
  val tx2 = {
    ...
    val tmp = Transaction(version = 1,
      ...
      txOut = TxOut(0.49 sys, Script.pay2wsh(redeemScript)) :: Nil,
      lockTime = 0
    )
  ...
  val tx3 = {
    val tmp: Transaction = Transaction(version = 1,
      ...
      txOut = TxOut(0.48 sys, Script.pay2wpkh(pub1)) :: Nil,
      lockTime = 0
    )
...
```scala
  ...
  // this is a standard tx that sends 0.4 SYS to D6YX7dpieYu8j1bV8B4RgksNmDk3sNJ4Ap
  ...
  // now let's create a simple tx that spends tx1 and send 0.39 SYS to P2WPK output
  val tx2 = {
    val tmp = Transaction(version = 1,
      txIn = TxIn(OutPoint(tx1.hash, 0), sequence = 0xffffffffL, signatureScript = Nil, witness = ScriptWitness.empty) :: Nil,
      txOut = TxOut(0.39 sys, Script.pay2wpkh(pub1)) :: Nil,
      lockTime = 0
    )
    Transaction.sign(tmp, Seq(SignData(tx1.txOut(0).publicKeyScript, priv1)))
  }
  ...
  val tx3 = {
    val tmp: Transaction = Transaction(version = 1,
      ...
      txOut = TxOut(0.38 sys, Script.pay2wpkh(pub1)) :: Nil, // we reuse the same output script but if could be anything else
      lockTime = 0
    )
...
```scala
  ...
  // this tx send 0.5 sys to our p2shaddress
  ...
  val tx1 = {
    val tmp: Transaction = Transaction(version = 1,
      ...
      txOut = TxOut(0.49 sys, OP_0 :: OP_PUSHDATA(pub1.hash160) :: Nil) :: Nil,
      lockTime = 0
    )
    val pubKeyScript = Script.pay2pkh(pub1)
...
Syscoin-lib provides and simple and complete implementation of BIP32 and BIP39.

#### HD Wallet (BIP32)

Let's play with the scala console and the first test vector from https://github.com/syscoin/bips/blob/master/bip-0032.mediawiki
...
[INFO]
[INFO] ------------------------------------------------------------------------
[INFO] Building syscoin-lib 0.9.4-SNAPSHOT
[INFO] ------------------------------------------------------------------------
[INFO]
[INFO] --- scala-maven-plugin:3.2.0:console (default-cli) @ syscoin-lib_2.11 ---
[WARNING]  Expected all dependencies to require Scala version: 2.11.4
[WARNING]  fr.acinq:syscoin-lib_2.11:0.9.4-SNAPSHOT requires scala version: 2.11.4
...
scala> import fr.acinq.syscoin.DeterministicWallet
DeterministicWallet   DeterministicWalletSpec

scala> import fr.acinq.syscoin.DeterministicWallet._
import fr.acinq.syscoin.DeterministicWallet._

scala> val m = generate(fromHexString("000102030405060708090a0b0c0d0e0f"))
m: fr.acinq.syscoin.DeterministicWallet.ExtendedPrivateKey = ExtendedPrivateKey(e8f32e723decf4051aefac8e2c93c9c5b214313817cdb01a1494b917c8436b35,873dff81c02f525623fd1fe5167eac3a55a049de3d314bb42ee227ffed37d508,0,m,0)
...
scala> publicKey(m)
res2: fr.acinq.syscoin.DeterministicWallet.ExtendedPublicKey = ExtendedPublicKey(0339a36013301597daef41fbe593a02cc513d0b55527ec2df1050e2e8ff49c85c2,873dff81c02f525623fd1fe5167eac3a55a049de3d314bb42ee227ffed37d508,0,m,0)
...
priv: fr.acinq.syscoin.DeterministicWallet.ExtendedPrivateKey = ExtendedPrivateKey(0f479245fb19a38a1954c5c7c0ebab2f9bdfd96a17563ef28a6a4b1a2a764ef4,cfb71883f01676f587d023cc53a35bc7f88f724b1f8c2892ac1275ac822a3edd,4,m/0h/1/2h/2,4001020172)
...
k2: fr.acinq.syscoin.DeterministicWallet.ExtendedPrivateKey = ExtendedPrivateKey(cbce0d719ecf7431d88e6a89fa1483e02e35092
...
K2: fr.acinq.syscoin.DeterministicWallet.ExtendedPublicKey = ExtendedPublicKey(0357bfe1e341d01c69fe5654309956cbea516822f
...
res8: fr.acinq.syscoin.DeterministicWallet.ExtendedPublicKey = ExtendedPublicKey(022a471424da5e657499d1ff51cb43c47481a03b1e77f951fe64cec9f5a48f7011,c783e67b921d2beb8f6b389cc646d7263b4145701dadd2161548a8b078e65e9e,5,m/0h/1/2h/2/1000000000,3632322520)
...
[WARNING]  fr.acinq:syscoin-lib_2.11:0.9.4-SNAPSHOT requires scala version: 2.11.6
...
key: fr.acinq.syscoin.BinaryData = b5b6d0127db1a9d2226af0c3346031d77af31e918dba64287a1b44b8ebf63cdd52676f672a290aae502472cf2d602c051f3e6f18055e84e4c43
897fc4e51a6ff
```
