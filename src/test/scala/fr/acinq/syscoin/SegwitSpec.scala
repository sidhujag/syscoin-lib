...
import fr.acinq.syscoin.Crypto.PrivateKey
...
class SegwitSpec extends FunSuite {
  ...
  test("tx serialization with witness") {
    // see https://github.com/syscoin/bips/blob/master/bip-0143.mediawiki#Example
  ...
  test("create p2pkh embedded in p2sh") {
    ...
    val tx1 = {
      val tmp: Transaction = Transaction(version = 1,
        ...
        txOut = TxOut(0.49 sys, OP_0 :: OP_PUSHDATA(Crypto.hash160(pub1.toBin)) :: Nil) :: Nil,
        lockTime = 0
      )
      val pubKeyScript = Script.pay2pkh(pub1)
