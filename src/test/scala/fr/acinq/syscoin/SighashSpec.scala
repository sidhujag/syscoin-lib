...
class SighashSpec extends FunSuite {
  test("SIGHASH_ANYONECANPAY lets you add inputs") {
    ...
    val previousTx = Seq(
      Transaction(version = 2, txIn = Nil, txOut = TxOut(42 millisys, Script.pay2pkh(publicKeys(0))) :: Nil, lockTime = 0),
      Transaction(version = 2, txIn = Nil, txOut = TxOut(42 millisys, Script.pay2pkh(publicKeys(1))) :: Nil, lockTime = 0)
    )

    // create a tx with no inputs
    val tx = Transaction(version = 2, txIn = Nil, txOut = TxOut(80 millisys, Script.pay2wsh(Script.createMultiSigMofN(2, publicKeys))) :: Nil, lockTime = 0L)
    ...
    val tx3 = tx2.copy(txOut = tx2.txOut.updated(0, tx2.txOut(0).copy(amount = 40 millisys)))
  ...
  test("SIGHASH_ANYONECANPAY lets you add inputs (SEGWIT version") {
    ...
    val previousTx = Seq(
      Transaction(version = 2, txIn = Nil, txOut = TxOut(42 millisys, Script.pay2wpkh(publicKeys(0))) :: Nil, lockTime = 0),
      Transaction(version = 2, txIn = Nil, txOut = TxOut(42 millisys, Script.pay2wpkh(publicKeys(1))) :: Nil, lockTime = 0)
    )
