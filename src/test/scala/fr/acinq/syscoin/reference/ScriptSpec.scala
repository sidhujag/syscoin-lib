...
/**
  * syscoin core reference script tests
  * see syscoin/src/test/script_tests.cpp for implementation details
  */
...
object ScriptSpec {
  ...
  def creditTx(scriptPubKey: Array[Byte], amount: Sys) = Transaction(version = 1,
  ...
  // use 0 sys if no amount is specified
  def runTest(witnessText: Seq[String], scriptSigText: String, scriptPubKeyText: String, flags: String, comments: Option[String], expectedText: String): Unit =
    runTest(witnessText, 0 sys, scriptSigText, scriptPubKeyText, flags, comments, expectedText)

  def runTest(witnessText: Seq[String], amount: Sys, scriptSigText: String, scriptPubKeyText: String, flags: String, comments: Option[String], expectedText: String): Unit = {
  ...
  def runTest(json: JValue): Int = {
    ...
    json.extract[List[List[JValue]]].tail.foreach(_ match {
      ...
      case JArray(m) :: JString(scriptSig) :: JString(scriptPubKey) :: JString(flags) :: JString(expected) :: JString(comments) :: Nil =>
        ...
        val amount: Sys = m.last match {
          case JDouble(value) => Sys(value)
