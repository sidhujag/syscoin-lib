...
import fr.acinq.syscoin.Crypto.{Point, PrivateKey, PublicKey, Scalar}
import fr.acinq.syscoin.Protocol._

/**
  * see https://github.com/syscoin/bips/blob/master/bip-0032.mediawiki
  */
object DeterministicWallet {
  ...
  def generate(seed: Seq[Byte]): ExtendedPrivateKey = {
    val I = Crypto.hmac512("Syscoin seed".getBytes("UTF-8"), seed)
    val IL = I.take(32)
