...
object Crypto {
  ...
  /**
    * 160 bits syscoin hash, used mostly for address encoding
    * hash160(input) = RIPEMD160(SHA256(input))
    *
    * @param input array of byte
    * @return the 160 bits SYS hash of input
    */
  def hash160(input: Seq[Byte]) = ripemd160(sha256(input))

  /**
    * 256 bits syscoin hash
    * hash256(input) = SHA256(SHA256(input))
    *
    * @param input array of byte
    * @return the 256 bits SYS hash of input
    */
  def hash256(input: Seq[Byte]) = sha256(sha256(input))

  /**
    * An ECDSA signature is a (r, s) pair. Syscoin uses DER encoded signatures
    *
    * @param r first value
  ...
  /**
    ...
    * @param privateKey private key. If you are using syscoin "compressed" private keys make sure to only use the first 32 bytes of
